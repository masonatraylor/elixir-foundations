...
function createTree(x, z) {
    ...
    // Add some random rotation and scale to make them look organic
...
function createSupervisor(x, z) {
    ...
    const label = createLabel('Supervisor', x, 8, z);
    return { mesh: group, name: 'Supervisor', radar: radarGeo, height: 6, x, z, label };
}

// 2. GenServer (State Core)
function createGenServer(x, z) {
    ...
    // An inner glowing sphere representing state
    ...
    const label = createLabel('GenServer (State)', x, 5.5, z);
    return { mesh: group, stateCore: stateCore, coreMesh: core, name: 'GenServer', height: 4, x, z, label };
}

// 3. Workers (Mini Factories)
function createWorker(x, z, name) {
    ...
    const label = createLabel(name, x, 3.5, z);
    const worker = { mesh: group, stack: stack, name: name, status: 'alive', height: 3, x, z, label, originalY: group.position.y };
    entities.push(worker);
    return worker;
}

// Draw a simple path linking them
...
// Layout
createPath(-4, 0, 2, 12); // vertical line to supervisor
createPath(0, 4, 10, 2); // horizontal line connecting workers
createPath(4, 0, 2, 10); // vertical line to GenServer
createPath(0, -4, 10, 2); // cross line
...
function spawnMessage(fromEntity, toEntity, type) {
    ...
    messages.push({
        ...
        progress: 0,
        type: type,
        speed: 0.015 + Math.random() * 0.005 // vary speed
    });
}
...
function onMessageArrived(msg) {
    ...
    if (msg.to.name === 'GenServer') {
        // State update effect: Inner core expands and changes color briefly
    ...
    if (msg.to.name.startsWith('Worker') && msg.type === 'restart') {
        // Restart effect: Pop back to normal scale
        msg.to.status = 'alive';
        msg.to.label.div.textContent = msg.to.name;
        ...
        new TWEEN.Tween(msg.to.mesh.position)
            .to({ y: msg.to.originalY }, 300)
...
function tryCrashWorker() {
    ...
    if (aliveWorkers.length > 0 && Math.random() < 0.4) {
        ...
        worker.status = 'dead';
        worker.label.div.textContent = 'CRASHED';
        worker.label.div.classList.add('dead');
        
        // Crash squash-and-stretch animation
        new TWEEN.Tween(worker.mesh.scale)
            .to({ x: 1.3, y: 0.1, z: 1.3 }, 200) // squish flat
            .easing(TWEEN.Easing.Quadratic.Out)
            .start();
            
        new TWEEN.Tween(worker.mesh.position)
            .to({ y: worker.originalY - 0.6 }, 200) // drop to ground
            .easing(TWEEN.Easing.Quadratic.Out)
            .start();
            
        // Supervisor notices and sends a restart package
        setTimeout(() => {
...
function trySendMessages() {
    ...
    if (Math.random() < 0.25) {
        ...
        if (aliveWorkers.length > 0) {
            ...
            spawnMessage(genserverEntity, target, 'normal');
        }
    }
}

// Timers
setInterval(tryCrashWorker, 3000);
setInterval(trySendMessages, 600);
...
function animate(time) {
    ...
    // Rotate supervisor radar
    if (!isPaused) {
        supervisorEntity.radar.rotation.y += 0.05;
        
        // Slowly pulse genserver core
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
            ...
            if (msg.progress >= 1) {
                ...
                const startX = msg.from.x;
                const startZ = msg.from.z;
                const endX = msg.to.x;
                const endZ = msg.to.z;
                ...
                // Tumble the message in the air
                msg.mesh.rotation.x += 0.1;
                msg.mesh.rotation.y += 0.15;
            }
        }
    }
    ...
    labels.forEach(l => {
        ...
        if (vec.z > 1) {
            l.div.style.opacity = '0'; // Behind camera (rare in orthographic but safe to check)
        } else {
